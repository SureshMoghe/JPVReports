...
@Component({
  selector: 'app-farmer-registration-mentor',
  templateUrl: './farmer-registration-mentor.component.html',
  styleUrls: ['./farmer-registration-mentor.component.css'],
})
export class FarmerRegistrationMentorComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.farmerAPI.mentorLevelReport(req);
      if (res.success) {
        this.excelData = [];
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Farmer Registration Mentor Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'mentorLevelFarmerRegReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.farmerAPI.mentorLevelPDFReport(req);
      if (res.success) {
        basePDF = res.result;
