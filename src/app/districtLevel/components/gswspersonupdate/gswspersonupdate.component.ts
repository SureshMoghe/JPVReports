...
@Component({
  selector: 'app-gswspersonupdate',
  templateUrl: './gswspersonupdate.component.html',
  styleUrls: ['./gswspersonupdate.component.css']
})
export class GswspersonupdateComponent implements OnInit {
  ...
  async btnUidDetails(): Promise<void> {
    try {
      ...
      if (this.utils.isEmpty(this.uidNum)) {    //this.uidNum
        this.toast.warning('Please Enter Aadhaar Number');
        return;
      }

      if (!this.utils.validateVerhoeff(this.uidNum)) {    //this.uidNum
        this.toast.warning('Please Enter Valid Aadhaar Number');
        return;
      }

      const req = {
        uidNum:this.uidNum,// this.uidNum,
      };
      this.spinner.show();
