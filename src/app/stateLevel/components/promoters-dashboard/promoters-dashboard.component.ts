  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  async loadDashboard(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.StateAPI.promotersHODashboard(req);
      this.spinner.hide();
      if (res.success) {
  ...
  async btnMdacAccountDashboardDetails(obj): Promise<void> {
    try {
      ...
      if (this.requestType === null) {
        ...
        res = await this.StateAPI.promotersPendingList(req);
      } else if (this.requestType === '1') {
        this.headingText = 'APPROVED LIST';
        res = await this.StateAPI.promotersApprovedList(req);
      } else if (this.requestType === '2') {
        this.headingText = 'REJECTED  LIST';
        res = await this.StateAPI.promotersRejectedList(req);
      }
      this.spinner.hide();
