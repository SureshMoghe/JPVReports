...
@Component({
    selector: 'app-mdss-dash-board-attendance-submitted-count',
    templateUrl: './mdss-dash-board-attendance-submitted-count.component.html',
    styleUrls: ['./mdss-dash-board-attendance-submitted-count.component.css']
})
export class MdssDashBoardAttendanceSubmittedCountComponent implements OnInit {
    CommonList: any[] = [];
    CommonExcelData: any[] = [];
            ...
            if (res.success) {
                ...
                for (var i = 0; i < this.CommonList.length; i++) {
                    let singleRow =
                    {
                        ...
                        DATE_OF_MEETING: this.CommonList[i].ATTEND_DATE,
                        NO_OF_PARTICIPANTS: this.CommonList[i].NO_OF_PART,
        ...
        try {
            ...
            if (res.success) {
                ...
                this.utils.JSONToCSVConvertor(
                    this.SecondCommonExcelData,
                    'Details Of Attendance Submitted' + '(' + obj.RBK_NAME + ')' + ' Report ' + this.session.dpTodayDate,
                    true
                );
    ...
    btnExcel(): void {
        this.utils.JSONToCSVConvertor(
            this.CommonExcelData,
            'Attendance Submitted Report ' + this.session.dpTodayDate,
            true
        );
    }
    ngOnDestroy(): void {
