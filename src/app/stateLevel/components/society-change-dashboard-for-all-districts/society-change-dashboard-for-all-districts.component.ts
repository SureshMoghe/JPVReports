...
export class SocietyChangeDashboardForAllDistrictsComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    async loadDistrictList(): Promise<void> {
        try {
            ...
            debugger;
            this.spinner.show();
            const response = await this.state.SmsSendingCredGet(req);
            this.spinner.hide();
            if (response.success) {
    ...
    async loadDashboard(): Promise<void> {
        try {
            ...
            this.spinner.show();
            const res = await this.state.HODashboard(req);
            this.spinner.hide();
            if (res.success) {
    ...
    async DistrictChanges(): Promise<void> {
        try {
            ...
            if (this.requestType === null) {
                ...
                res = await this.state.HOPendingList(req);
            } else if (this.requestType === '1') {
                this.headingText = 'APPROVED LIST';
                res = await this.state.HOApprovedList(req);
            } else if (this.requestType === '2') {
                this.headingText = 'REJECTED  LIST';
                res = await this.state.HORejectedList(req);
            }
            this.spinner.hide();
            this.societyChangeList = [];
            if (res.success) {
    ...
    async btnVerify(obj): Promise<void> {
        try {
            ...
            this.spinner.show();
            const response = await this.state.TransactionsByUnionidGet(req); //await this.districtHOAPI.societyHOUpdate(req);
            if (response.success) {
