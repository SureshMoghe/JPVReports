...
bmcufilehide: boolean = false;
  ...
  async loadDevices(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const response = await this.StateAPI.TechnisianDetails_Select(req);
      this.spinner.hide();
      if (response.success) {
 ...
 async onbmculocchange(obj): Promise<void> {   debugger;
  ...
  async btnSubmit(): Promise<void> {
    try {  debugger;
      if (this.validate()) { 
        ...
        this. operateyear = this.operatedyear.substring(6,10);

          debugger;
        this.calibrationDeviceDetails.operatedyear = this.operateyear;
        const req = {  
          type:1,   
        ...
        this.spinner.show();
        const response = await this.StateAPI.BMCUDeviceDetailsIns(req);
        if (response.success) {
...
debugger;
  ...
  debugger;
      const req={
        ...
        bmculist: [{DEVICE_ID :this.deviceId,
      ...
      async LoadBMCUReport(): Promise<void> { 
        try {   debugger;
          ...
          if (response.success) {
            ...
            }else{ this.shiftedavail=false;this.issubmit=false;} 
...
debugger; 
      ...
      shiftedBMCUChange(obj){  debugger;
