...
export class FarmerBankBAUStatusRbkReportComponent implements OnInit {
  ...
  onInvalidAccChange(result): void {
    this.router.navigate(['/state/FarmerBankInvalidAccountsReport'], {
      queryParams: { request: result },
    });
  }

  onVerifiedApprovedRejectedChange(result): void {
    this.router.navigate(['/state/FarmerBankVerifiedAcceptedRejectedReport'], {
      queryParams: { request: result },
    });
  }

  onPendingAtDAChange(result): void {
    this.router.navigate(['/state/FarmerBankPendingAtDaReport'], {
      queryParams: { request: result },
    });
  }

  onPendingAtMentorChange(result): void {
    this.router.navigate(['/state/FarmerBankPendingAtMentorReport'], {
      queryParams: { request: result },
    });
  }
