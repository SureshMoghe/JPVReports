...
@Component({
  selector: 'app-bmcu-construction-mentor',
  templateUrl: './bmcu-construction-mentor.component.html',
  styleUrls: ['./bmcu-construction-mentor.component.css'],
})
export class BmcuConstructionMentorComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.constructionAPI.mentorLevelReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.mentorLevelDetails.length; i++) {
          ...
          this.reportTotals.FEASIB_OF_LAND_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FEASIB_OF_LAND_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FEASIB_OF_LAND_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FEASIB_OF_LAND_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.REGIS_OF_LAND_SUBMIT += parseInt(
            this.mentorLevelDetails[i].REGIS_OF_LAND_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.REGIS_OF_LAND_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].REGIS_OF_LAND_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FACIL_AT_BUILD_SITE_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FACIL_AT_BUILD_SITE_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FACIL_AT_BUILD_SITE_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FACIL_AT_BUILD_SITE_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.BUILD_BASE_LEVEL_SUBMIT += parseInt(
            this.mentorLevelDetails[i].BUILD_BASE_LEVEL_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.BUILD_BASE_LEVEL_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].BUILD_BASE_LEVEL_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.BUILD_BASE_LEVEL_PARTI_SUBMIT += parseInt(
            this.mentorLevelDetails[i].BUILD_BASE_LEVEL_PARTI_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.SUPER_STRUCTURE_SUBMIT += parseInt(
            this.mentorLevelDetails[i].SUPER_STRUCTURE_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.SUPER_STRUCTURE_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].SUPER_STRUCTURE_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.SUPER_STRUC_SUBMIT_PART_SUBMIT += parseInt(
            this.mentorLevelDetails[i].SUPER_STRUC_SUBMIT_PART_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FINISH_LEVEL_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FINISH_LEVEL_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FINISH_LEVEL_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FINISH_LEVEL_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FINISH_LEVEL_PART_SUBMIT += parseInt(
            this.mentorLevelDetails[i].FINISH_LEVEL_PART_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.OTHER_INFRASTRAC_SUBMIT += parseInt(
            this.mentorLevelDetails[i].OTHER_INFRASTRAC_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.OTHER_INFRASTRAC_NOT_SUBMIT += parseInt(
            this.mentorLevelDetails[i].OTHER_INFRASTRAC_NOT_SUBMIT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.OTHER_INFRASTRAC_PARTI_SUBMIT += parseInt(
            this.mentorLevelDetails[i].OTHER_INFRASTRAC_PARTI_SUBMIT
          );
          let singleRow = {
            S_NO: i + 1,
            ...
            SUPER_STRUC_SUBMIT_PART_SUBMIT:
              this.mentorLevelDetails[i].SUPER_STRUC_SUBMIT_PART_SUBMIT,
            FINISH_LEVEL_SUBMIT: this.mentorLevelDetails[i].FINISH_LEVEL_SUBMIT,
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'BMCU Construction Mentor Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'mentorLevelBMCUCOnstructionReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.constructionAPI.mentorLevelPDFReport(req);
      if (res.success) {
        basePDF = res.result;
