...
@Component({
  selector: 'app-farmer-sms-district',
  templateUrl: './farmer-sms-district.component.html',
  styleUrls: ['./farmer-sms-district.component.css'],
})
export class FarmerSmsDistrictComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.farmerSms.farmerSmsDistrictLevelReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.districtLevelDetails.length; i++) {
          ...
          this.reportTotals.FARMERS_REGISTERED += parseInt(
            this.districtLevelDetails[i].FARMERS_REGISTERED
          );
          // tslint:disable-next-line: radix
          this.reportTotals.FMR_REGIS_SMS_SENT += parseInt(
            this.districtLevelDetails[i].FMR_REGIS_SMS_SENT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.NO_OF_SCHEDULES_CREATED += parseInt(
            this.districtLevelDetails[i].NO_OF_SCHEDULES_CREATED
          );
          // tslint:disable-next-line: radix
          this.reportTotals.SCHEDULE_INVITATION_SENT += parseInt(
            this.districtLevelDetails[i].SCHEDULE_INVITATION_SENT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.VV_INVITED += parseInt(
            this.districtLevelDetails[i].VV_INVITED
          );
          // tslint:disable-next-line: radix
          this.reportTotals.VV_INVITED_PENDING += parseInt(
            this.districtLevelDetails[i].VV_INVITED_PENDING
          );
          // tslint:disable-next-line: radix
          this.reportTotals.TOTAL_NO_OF_PARTICIPANTS += parseInt(
            this.districtLevelDetails[i].TOTAL_NO_OF_PARTICIPANTS
          );
          let singleRow = {
            S_NO: i + 1,
            ...
            FARMERS_REGISTERED: this.districtLevelDetails[i].FARMERS_REGISTERED,
            FMR_REGIS_SMS_SENT: this.districtLevelDetails[i].FMR_REGIS_SMS_SENT,
            NO_OF_SCHEDULES_CREATED: this.districtLevelDetails[i]
              .NO_OF_SCHEDULES_CREATED,
            SCHEDULE_INVITATION_SENT: this.districtLevelDetails[i]
              .SCHEDULE_INVITATION_SENT,
            VV_INVITED: this.districtLevelDetails[i].VV_INVITED,
            VV_INVITED_PENDING: this.districtLevelDetails[i].VV_INVITED_PENDING,
            TOTAL_NO_OF_PARTICIPANTS: this.districtLevelDetails[i]
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Farmer SMS District Level Report',
      true
    );
  }
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'districtLevelFarmerSmsReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.farmerSms.farmerSmsDistrictLevelReportPDF(req);
      if (res.success) {
        basePDF = res.result;
