...
@Component({
  selector: 'app-land-allotment-mandal',
  templateUrl: './land-allotment-mandal.component.html',
  styleUrls: ['./land-allotment-mandal.component.css'],
})
export class LandAllotmentMandalComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.allotmentAPI.landAllotmentMandalReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.mandalLevelDetails.length; i++) {
          let singleRow = {
            S_NO: i + 1,
            RBKName:this.mandalLevelDetails[i].RBK_NAME,
  ...
  async btnPdfView(pdf): Promise<void> {
    try {
if(pdf===''||pdf===null||pdf===undefined||pdf==='NOT SUBMITTED')
{
  this.toast.showImage("PDF NOT UPLOADED"); return;
}
