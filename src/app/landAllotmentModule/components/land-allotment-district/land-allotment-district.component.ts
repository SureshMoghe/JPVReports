...
@Component({
  selector: 'app-land-allotment-district',
  templateUrl: './land-allotment-district.component.html',
  styleUrls: ['./land-allotment-district.component.css'],
})
export class LandAllotmentDistrictComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.allotmentAPI.landAllotmentDistrictReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.districtLevelDetails.length; i++) {
          ...
          let singleRow = {
            ...
            TOTAL_LAND_SUBMITTED: this.districtLevelDetails[i]
              .TOTAL_LAND_SUBMITTED,
            PERCENTAGE_COMPLETED: this.districtLevelDetails[i]
              .PERCENTAGE_COMPLETED,
            TOTAL_LAND_NOT_SUBMITTED: this.districtLevelDetails[i]
              .TOTAL_LAND_NOT_SUBMITTED,
            PERCENTAGE_NOT_COMPLETED: this.districtLevelDetails[i]
              .PERCENTAGE_NOT_COMPLETED,
            TOTAL_LAND_HANDOVER_SUBMIT: this.districtLevelDetails[i]
              .TOTAL_LAND_HANDOVER_SUBMIT,
            TOTAL_LAND_HANDOVER_NOT_SUBMIT: this.districtLevelDetails[i]
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'districtLevelLandAllotment';
      let basePDF = '';
      this.spinner.show();
