...
@Component({
  selector: 'app-grounding-ah-level-login',
  templateUrl: './grounding-ah-level-login.component.html',
  styleUrls: ['./grounding-ah-level-login.component.css'],
})
export class GroundingAhLevelLoginComponent
  implements OnInit, OnDestroy, AfterViewInit {
  // tslint:disable-next-line: no-output-on-prefix
  @Output() onMandalNameChange = new EventEmitter<string>();
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const req = {
        districtId: this.session.mentorId,
      };
      const res = await this.groundingAPI.ahLevelLoginReport(req);
      if (res.success) {
        this.excelData = [];
  ...
  btnGetDetails(obj): void {
    ...
    this.onMandalNameChange.emit(encryptedString);
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      let basePDF = '';
      this.spinner.show();
      const res = await this.groundingAPI.ahLevelLoginPDFReport(req);
      if (res.success) {
        basePDF = res.result;
