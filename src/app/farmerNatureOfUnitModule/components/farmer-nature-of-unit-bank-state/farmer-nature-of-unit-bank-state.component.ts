...
@Component({
  selector: 'app-farmer-nature-of-unit-bank-state',
  templateUrl: './farmer-nature-of-unit-bank-state.component.html',
  styleUrls: ['./farmer-nature-of-unit-bank-state.component.css'],
})
export class FarmerNatureOfUnitBankStateComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.farmerNOUAPI.farmerNOUBankState();
      if (res.success) {
        this.excelData = [];
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Farmer Nature Of Unit Bank State Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      const fileName = 'bankStateLevelNatureOfUnit';
      let basePDF = '';
      this.spinner.show();
      const res = await this.farmerNOUAPI.bankStateNOUPDFReport();
      if (res.success) {
        basePDF = res.result;
