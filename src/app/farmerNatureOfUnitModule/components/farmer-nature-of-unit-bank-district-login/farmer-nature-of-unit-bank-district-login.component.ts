...
@Component({
  selector: 'app-farmer-nature-of-unit-bank-district-login',
  templateUrl: './farmer-nature-of-unit-bank-district-login.component.html',
  styleUrls: ['./farmer-nature-of-unit-bank-district-login.component.css'],
})
export class FarmerNatureOfUnitBankDistrictLoginComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.farmerNOUAPI.farmerNOUBankDistrictLogin(req);
      if (res.success) {
        this.excelData = [];
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Farmer Nature Of Unit Bank Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      const fileName = 'bankLevelNatureOfUnit';
      let basePDF = '';
      const req = {
        districtId: this.session.districtId,
      };
      this.spinner.show();
      const res = await this.farmerNOUAPI.bankDistrictLoginNOUPDFReport(req);
      if (res.success) {
        basePDF = res.result;
