...
@Component({
  selector: 'app-building-inspection-mentor',
  templateUrl: './building-inspection-mentor.component.html',
  styleUrls: ['./building-inspection-mentor.component.css'],
})
export class BuildingInspectionMentorComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.buildingInspect.mentorLevelReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.mentorLevelDetails.length; i++) {
          ...
          this.reportTotals.TOTAL_BUILDING_INSPECT += parseInt(
            this.mentorLevelDetails[i].TOTAL_BUILDING_INSPECT
          );
          // tslint:disable-next-line: radix
          this.reportTotals.TOTAL_BUILDING_NOT_INSPECT += parseInt(
            this.mentorLevelDetails[i].TOTAL_BUILDING_NOT_INSPECT
          );
          let singleRow = {
            S_NO: i + 1,
            ...
            TOTAL_BUILDING_INSPECT: this.mentorLevelDetails[i]
              .TOTAL_BUILDING_INSPECT,
            TOTAL_BUILDING_NOT_INSPECT: this.mentorLevelDetails[i]
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Building Inspection Mentor Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'mentorLevelBuildingInspectionReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.buildingInspect.mentorLevelInspectPDFReport(req);
      if (res.success) {
        basePDF = res.result;
