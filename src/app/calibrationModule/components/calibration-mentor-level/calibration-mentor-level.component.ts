...
@Component({
  selector: 'app-calibration-mentor-level',
  templateUrl: './calibration-mentor-level.component.html',
  styleUrls: ['./calibration-mentor-level.component.css'],
})
export class CalibrationMentorLevelComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.calibration.mentorLevelReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.mentorLevelDetails.length; i++) {
          ...
          let singleRow = {
            ...
            TOTAL_CALIB_DONE_VILLAGES: this.mentorLevelDetails[i]
              .TOTAL_CALIB_DONE_VILLAGES,
            TOTAL_CALIB_NOT_DONE_VILLAGES: this.mentorLevelDetails[i]
  ...
  getExcelDownload(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Calibration Mentor Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'mentorLevelCalibrationReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.calibration.mentorLevelCalibrationPDFReport(req);
      if (res.success) {
        basePDF = res.result;
