...
@Component({
  selector: 'app-calibration-state-level',
  templateUrl: './calibration-state-level.component.html',
  styleUrls: ['./calibration-state-level.component.css'],
})
export class CalibrationStateLevelComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.calibration.stateLevelReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.stateLevelDetails.length; i++) {
          ...
          let singleRow = {
            ...
            TOTAL_CALIB_DONE_VILLAGES: this.stateLevelDetails[i]
              .TOTAL_CALIB_DONE_VILLAGES,
            TOTAL_CALIB_NOT_DONE_VILLAGES: this.stateLevelDetails[i]
  ...
  getExcelDownload(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Calibration State Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'stateLevelCalibrationReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.calibration.stateLevelCalibrationPDFReport(req);
      if (res.success) {
        basePDF = res.result;
