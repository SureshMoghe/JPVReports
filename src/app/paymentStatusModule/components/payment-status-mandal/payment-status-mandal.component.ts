...
@Component({
  selector: 'app-payment-status-mandal',
  templateUrl: './payment-status-mandal.component.html',
  styleUrls: ['./payment-status-mandal.component.css'],
})
export class PaymentStatusMandalComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.paymentAPI.paymentMandalReport(req);
      if (res.success) {
        this.excelData = [];
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Payment Status Mandal Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'mandalLevelPaymentStatusReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.paymentAPI.paymentMandalPDFReport(req);
      if (res.success) {
        basePDF = res.result;
