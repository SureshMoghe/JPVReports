...
@Component({
  selector: 'app-vv-farmer-data-district',
  templateUrl: './vv-farmer-data-district.component.html',
  styleUrls: ['./vv-farmer-data-district.component.css'],
})
export class VvFarmerDataDistrictComponent
  ...
  async loadReport(): Promise<void> {
    try { debugger;
      ...
      this.spinner.show();
      const res = await this.vvFarmerAPI.vvFarmerDataDistrictReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.districtLevelDetails.length; i++) {
          ...
          this.reportTotals.TOTAL_ROUTE_NOS += parseInt(
            this.districtLevelDetails[i].TOTAL_ROUTE_NOS
          );
          // tslint:disable-next-line: radix
          ...
          this.reportTotals.TOTAL_VOLUNTEERS += parseInt(
            this.districtLevelDetails[i].TOTAL_VOLUNTEERS
          );
          // tslint:disable-next-line: radix
          ...
          this.reportTotals.TOTAL_FARMERS_WITH_OUT_AINM += parseInt(
            this.districtLevelDetails[i].TOTAL_FARMERS_WITH_OUT_AINM
          );
          // tslint:disable-next-line: radix
          this.reportTotals.TOTAL_FARMERS_WITH_AINM += parseInt(
            this.districtLevelDetails[i].TOTAL_FARMERS_WITH_AINM
          );

          // tslint:disable-next-line: radix
          this.reportTotals.TOTAL_MIGRATED += parseInt(
            this.districtLevelDetails[i].MIGRATED
          );
          // tslint:disable-next-line: radix
          this.reportTotals.TOTAL_DEAD += parseInt(
            this.districtLevelDetails[i].DEAD
          );
          ...
          this.reportTotals.TOTAL_APPROVED_FMR += parseInt(
            this.districtLevelDetails[i].TOTAL_APPROVED_FMR
          );
          // tslint:disable-next-line: radix
          this.reportTotals.TOTAL_APPROVED_FMR_PERCNT += parseFloat(
            this.districtLevelDetails[i].TOTAL_APPROVED_FMR_PERCNT
          );
          // tslint:disable-next-line: radix
          ...
          this.reportTotals.AH_DEPT_VV_ANM_YES += parseInt(
            this.districtLevelDetails[i].AH_DEPT_VV_ANM_YES
          );
          // tslint:disable-next-line: radix
          this.reportTotals.AH_DEPT_YES_VV_ANM_NO += parseInt(
            this.districtLevelDetails[i].AH_DEPT_YES_VV_ANM_NO
          );
          let singleRow = {
            S_NO: i + 1,
            MANDAL_NAME: this.districtLevelDetails[i].MANDAL_NAME,
            TOTAL_ROUTE_NOS: this.districtLevelDetails[i].TOTAL_ROUTE_NOS,
            TOTAL_RBK: this.districtLevelDetails[i].TOTAL_RBK,
            TOTAL_VILLAGE: this.districtLevelDetails[i].TOTAL_VILLAGE,
            TOTAL_VOLUNTEERS: this.districtLevelDetails[i].TOTAL_VOLUNTEERS,
            TOTAL_VV_LOGEDIN: this.districtLevelDetails[i].TOTAL_VV_LOGEDIN,
            TOTAL_FARMERS_WITH_OUT_AINM: this.districtLevelDetails[i]
              .TOTAL_FARMERS_WITH_OUT_AINM,
            TOTAL_FARMERS_WITH_AINM: this.districtLevelDetails[i]
              .TOTAL_FARMERS_WITH_AINM,
            MIGRATED:  this.districtLevelDetails[i].MIGRATED,
            DEAD:  this.districtLevelDetails[i].DEAD,
            TOTAL_APPROVED_FMR: this.districtLevelDetails[i].TOTAL_APPROVED_FMR,
            TOTAL_APPROVED_FMR_PERCNT: this.districtLevelDetails[i]
              .TOTAL_APPROVED_FMR_PERCNT,
            TOTAL_ANIMALS: this.districtLevelDetails[i].TOTAL_ANIMALS,
            AH_DEPT_VV_ANM_YES: this.districtLevelDetails[i].AH_DEPT_VV_ANM_YES,
            AH_DEPT_YES_VV_ANM_NO: this.districtLevelDetails[i]
        ...
        this.reportTotals.TOTAL_APPROVED_FMR_PERCNT =
          this.reportTotals.TOTAL_APPROVED_FMR_PERCNT /
          this.districtLevelDetails.length;
        this.reportTotals.TOTAL_APPROVED_FMR_PERCNT = parseFloat(
  ...
  btnExcel(): void {
    this.utils.JSONToCSVConvertor(
      this.excelData,
      'Volunteer Farmer Data District Level Report',
      true
    );
  }
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      const fileName = 'districtLevelVolunteerFarmerDataReport';
      let basePDF = '';
      this.spinner.show();
      const res = await this.vvFarmerAPI.vvFarmerDataDistrictPDFReport(req);
      if (res.success) {
        basePDF = res.result;
