...
@Component({
  selector: 'app-vv-farmer-data-rbk',
  templateUrl: './vv-farmer-data-rbk.component.html',
  styleUrls: ['./vv-farmer-data-rbk.component.css'],
})
export class VvFarmerDataRbkComponent
  ...
  async loadReport(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.vvFarmerAPI.vvFarmerDataRbkReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.rbkLevelDetails.length; i++) {
          ...
          this.reportTotals.TOTAL_FARMERS_WITH_AINM += parseInt(
           ...
           this.reportTotals.TOTAL_MIGRATED += parseInt(
            this.rbkLevelDetails[i].MIGRATED
          );
           // tslint:disable-next-line: radix
           this.reportTotals.TOTAL_DEAD += parseInt(
            this.rbkLevelDetails[i].DEAD
          );
          ...
          let singleRow = {
            ...
            TOTAL_FARMERS_WITH_AINM: this.rbkLevelDetails[i]
              ...
              MIGRATED:  this.rbkLevelDetails[i].MIGRATED,
              DEAD:  this.rbkLevelDetails[i].DEAD,
  ...
  async btnPDF(): Promise<void> {
    try {
      ...
      let basePDF = '';
      this.spinner.show();
      const res = await this.vvFarmerAPI.vvFarmerDataRbkPDFReport(req);
      if (res.success) {
        basePDF = res.result;
